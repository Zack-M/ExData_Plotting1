...
## plot4.png = Plotting 4 graphs in a pane 

png(filename="plot4.png", width=480, height=480, units="px", type = "quartz")

#setting the 2x2 panel
par(mfrow=c(2,2))

# Topleft plot  (i.e. 1,1)
plot(subpowcon$DateTime, subpowcon$Global_active_power, type='l', xlab="", ylab="Global Active Power (kilowatts)")


# Topright plot b (i.e. 1,2)
plot(subpowcon$DateTime, subpowcon$Voltage , type='l', xlab="datetime", ylab="Voltage")


# Bottomleft plot c (i.e. 2,1)
...
# Bottomright plot d (i.e. 2,2)
plot(subpowcon$DateTime, subpowcon$Global_reactive_power , type='l', xlab="datetime", ylab="Global_reactive_power")
